// Reads salary and ID from input, catching wrong-type input specifically.
// Concept: Specific exception handling — catching InputMismatchException explicitly.
import java.util.HashMap;
import java.util.Scanner;
import java.util.InputMismatchException;

class Employee{
    int id;
    String name;
    double salary;

    Employee(int id, String name, double salary){
        this.id = id; 
        this.name = name;
        this.salary = salary;
    }

    void setItems(double sal, int id){
        this.salary = sal;
        this.id = id;
    }

    void display(){
        System.out.println("ID: " + id + ", Name: " + name + ", Salary: " + salary);
    }
}

public class pgm41 {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        HashMap<Integer, Employee> employees = new HashMap<>();
        employees.put(101,new Employee(101,"Alice",200000));
        employees.put(202, new Employee(202, "John",250000));
        employees.put(303,new Employee(303,"Carol",185000));
        Employee e = employees.get(202);
        try{
            double sala = sc.nextDouble();
            int id = sc.nextInt();
            e.setItems(sala,id);
        }
        catch(InputMismatchException ex){
            System.out.println(ex);
        }
        employees.remove(303);
        for(Employee e1:employees.values()){e1.display();}
    }  
}
